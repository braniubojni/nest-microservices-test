...
async function bootstrap() {
  ...
  const logger = new Logger(ServiceBModule.name);
  ...
  const PORT = process.env.SERVICE_B_PORT || 5001;

  await app.listen(PORT, () => {
    logger.debug(`Server listens: ${PORT || 5001}`);
    logger.debug(`Swagger docs: http://localhost:${PORT || 5001}/api`);
  });
}
bootstrap();
